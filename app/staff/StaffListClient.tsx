...
function getTrendComment(
  ...
  if (isFirstEntry) return { text: '初回入力', color: 'text-[#8B94A7]' }
  if (salesDiff !== null && salesDiff >= 10) return { text: '📈 売上上昇傾向', color: 'text-emerald-400' }
  if (unitPriceDiff !== null && unitPriceDiff >= 5) return { text: '💰 単価上昇傾向', color: 'text-emerald-400' }
  ...
  return { text: '安定推移', color: 'text-[#8B94A7]' }
}
...
export default function StaffListClient({
  ...
  const loadData = useCallback(async (sid: string, week: string) => {
    ...
    const built: StaffCard[] = result.staffList.map((s) => {
      ...
      const prevUnitPrice = prevSales !== null && prevVisits !== null && prevVisits > 0 ? Math.round(prevSales / prevVisits) : null

      const salesDiff = sales !== null && prevSales !== null && prevSales > 0 ? ((sales - prevSales) / prevSales) * 100 : null
      const visitsDiff = visits !== null && prevVisits !== null && prevVisits > 0 ? ((visits - prevVisits) / prevVisits) * 100 : null
      const unitPriceDiff = unitPrice !== null && prevUnitPrice !== null && prevUnitPrice > 0 ? ((unitPrice - prevUnitPrice) / prevUnitPrice) * 100 : null
      ...
      {!fetching && storeId && (
        <>
          {cards.length === 0 ? (
            ...
            cards.map((c) => (
              <div key={c.id} className={`bg-[#111A2B] rounded-2xl p-4 border mb-3 ${c.isAlert ? 'border-red-500/30' : 'border-white/5'}`}>
                {/* ヘッダー */}
                <div className="flex items-center justify-between mb-1">
                  <p className="text-[#E6ECF5] text-lg font-bold">{c.name}</p>
                  {c.isAlert && (
                    <span className="text-xs bg-red-500/10 text-red-400 px-2 py-0.5 rounded-full">⚠️ 先週より大きく下落</span>
                  )}
                </div>
