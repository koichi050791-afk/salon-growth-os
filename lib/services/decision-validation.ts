...
export function normalizeDecisionValidationInput(
  input: DecisionValidationInput,
): NormalizedDecisionValidationInput | null {
  if (
    typeof input.decisionId !== 'string'
    || !AIRTABLE_RECORD_ID_PATTERN.test(input.decisionId)
  ) {
    return null
  }
  ...
  if (
    !outcomeObserved
    || typeof validationNote === 'undefined'
    || !isCompletedDecisionValidationState(input.validationState)
  ) {
    return null
  }
...
function mapReadError(
  error: GetAirtableDecisionResult['error'],
): Extract<SaveDecisionValidationResult, { ok: false }>['error'] {
...
export async function saveDecisionValidation(
  ...
  if (!normalized) {
    return { ok: false, saved: false, error: 'invalid_input' }
  }
  ...
  if (existing.error || !existing.data) {
    return {
      ok: false,
      saved: false,
      error: mapReadError(existing.error ?? 'request_failed'),
    }
  }

  if (existing.data.dataKind !== 'REAL') {
    return { ok: false, saved: false, error: 'not_real' }
  }

  if (!existing.data.values.nextObservation?.trim()) {
    return { ok: false, saved: false, error: 'not_open' }
  }

  if (existing.data.validation.validationState !== 'UNVALIDATED') {
    return { ok: false, saved: false, error: 'already_validated' }
  }
  ...
  if (!updated.ok) {
    return {
      ok: false,
      saved: false,
      error: updated.error === 'missing_config' ? 'missing_config' : 'persistence_failed',
    }
  }
...
function projectOpenValidation(
  decision: AirtableDecisionRecord,
): OpenDecisionValidationItem | null {
...
export async function listOpenDecisionValidations(
  ...
  return {
    data: result.data
      .map(projectOpenValidation)
      .filter((item): item is OpenDecisionValidationItem => item !== null),
    error: null,
  }
}
